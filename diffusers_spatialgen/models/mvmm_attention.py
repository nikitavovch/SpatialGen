...
from .mvmm_attention_processor import JointMVAttnProcessor2_0, XFormersMVAttnProcessor, XFormersJointAttnProcessor
...
# The only modifications: `AttnProcessor2_0` -> `CustomAttention` or 'CustomJointAttention'
...
@maybe_allow_in_graph
class BasicMVMMTransformerBlock(nn.Module):
    r"""
    ...
    def forward(
        ...
        # logger.info(f"[BasicMVMMTransformerBlock] attn1 hidden_states shape: {hidden_states.shape}")
        # joint attention twice
        if self.cd_attention_mid:
            if self.use_ada_layer_norm and self.norm_type == "ada_norm":
                norm_hidden_states = self.norm_joint_mid(hidden_states, timestep)
            elif self.norm_type == "ada_norm_single":
                # COPY the scale and shift from norm1
                ...
                raise ValueError("Incorrect norm used")
            # norm_hidden_states = self.norm_joint_mid(hidden_states, timestep) if self.use_ada_layer_norm else self.norm_joint_mid(hidden_states)
            mid_attn_output = self.attn_joint_mid(
                            norm_hidden_states,
                            num_input_views=self.num_input_views,
                            num_output_views=self.num_output_views,
        ...
        if self.attn2 is not None:
            ...
            hidden_states = attn_output + hidden_states
        # logger.info(f"[BasicMVMMTransformerBlock] attn2 hidden_states shape: {hidden_states.shape}")
