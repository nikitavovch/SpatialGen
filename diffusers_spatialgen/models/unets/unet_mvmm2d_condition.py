...
from .unet_mvmm2d_blocks import get_down_mvmmblock, get_mid_mvmmblock, get_up_mvmmblock
...
class UNetMVMM2DConditionModel(
    ...
    def __init__(
        ...
        num_input_views: int = 4,  # Number of input views, only for mutl-view attention
        num_output_views: int = 4,  # Number of output views, only for mutl-view attention
        num_tasks: int = 2,  # Number of tasks, only for mutl-modal attention
        cd_attention_mid: bool = True,  # whether do cross-domain attention in the middle of the Attnblock
        multiview_attention: bool = True,  # whether use multi-view attention
        sparse_mv_attention: bool = False,  # whether use sparse multi-view attention,
        disable_mv_attention_in_64x64: bool = False,  # whether disable multi-view attention in 64x64 resolution
    ):
        super().__init__()
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            # disable multi-view attention in 64x64 resolution
            if i == 0 and disable_mv_attention_in_64x64:
                tmp_multiview_attention = False
            else:
                tmp_multiview_attention = multiview_attention
                
            down_block = get_down_mvmmblock(
                down_block_type,
                ...
                num_output_views=num_output_views,
                num_tasks=num_tasks,
                cd_attention_mid=cd_attention_mid,
                multiview_attention=tmp_multiview_attention,
                sparse_mv_attention=sparse_mv_attention,
            )
            self.down_blocks.append(down_block)

        # mid
        self.mid_block = get_mid_mvmmblock(
            mid_block_type,
            temb_channels=blocks_time_embed_dim,
            ...
            dropout=dropout,
            num_input_views=num_input_views,  # Number of input views, only for mutl-view attention
            num_output_views=num_output_views,  # Number of output views, only for mutl-view attention
            num_tasks=num_tasks,  # Number of tasks, only for mutl-modal attention
            cd_attention_mid=cd_attention_mid,  # whether do cross-domain attention in the middle of the Attnblock
            multiview_attention=multiview_attention,  # whether use multi-view attention
            sparse_mv_attention=sparse_mv_attention,  # whether use sparse multi-view attention
        )
        ...
        for i, up_block_type in enumerate(up_block_types):
            ...
            # disable multi-view attention in 64x64 resolution
            if is_final_block and disable_mv_attention_in_64x64:
                tmp_multiview_attention = False
            else:
                tmp_multiview_attention = multiview_attention
                
            up_block = get_up_mvmmblock(
                up_block_type,
    ...
    def from_pretrained_new(
        ...
        view_concat_condition: bool = True,
        input_concat_plucker: bool = True,
        input_concat_binary_mask: bool = True,
        input_concat_warpped_image: bool = False,
        projection_class_embeddings_input_dim: int = 8,  # The dimension of the `class_labels` input when `class_embed_type="projection"`. Required when `class_embed_type="projection"`.
        num_input_views: int = 4,  # Number of input views, only for mutl-view attention
        num_output_views: int = 4,  # Number of output views, only for mutl-view attention
        num_tasks: int = 4,  # Number of multi-modal tasks
        cd_attention_mid: bool = True,  # whether do cross-domain attention in the middle of the Attnblock
        multiview_attention: bool = True,  # whether use multi-view attention
        sparse_mv_attention: bool = False,  # whether use sparse multi-view attention
        disable_mv_attention_in_64x64: bool = False,  # whether disable multi-view attention in 64x64
        ...
        **kwargs
        ...
        config["input_concat_warpped_image"] = input_concat_warpped_image

        config["num_input_views"] = num_input_views
        config["num_output_views"] = num_output_views
        ...
        config["class_embed_type"] = "projection"    # set class_embed_type to projection
        config["projection_class_embeddings_input_dim"] = projection_class_embeddings_input_dim
        config["disable_mv_attention_in_64x64"] = disable_mv_attention_in_64x64
        ...
        config["mid_block_type"] = "UNetMidBlockMVMM2DCrossAttn"
        ...
        if not from_scratch:
            ...
            if model.conv_in.weight.data.shape[1] != latent_channels:
                ...
                if zero_init_conv_in:
                    ...
                    print(f"Zero initialize conv_in layers with {model.conv_in.weight.data.shape} channels")
